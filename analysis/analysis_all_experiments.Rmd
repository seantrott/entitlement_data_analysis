...
```{r}
coded.reduced$agree_reason = coded.reduced$Reason.SB == coded.reduced$Just1.ST
mean(coded.reduced$agree_reason)

preface_bound = cbind(coded.reduced$Reason.SB, coded.reduced$Just1.ST)
preface_kappa = cohen.kappa(preface_bound)
preface_kappa
```


#### Set reason to JV's codes if disagreement

```{r}
coded_with_norms$reason_final = ifelse(coded_with_norms$Reason.JV %in% c(0, 1), 
                                       coded_with_norms$Reason.JV,
                                       coded_with_norms$Just1.ST)
```
...
```{r}
coded.reduced$agree_repay = coded.reduced$Repayment.SB == coded.reduced$Just2.ST
mean(coded.reduced$agree_repay)

preface_bound = cbind(coded.reduced$Repayment.SB, coded.reduced$Just2.ST)
preface_kappa = cohen.kappa(preface_bound)
preface_kappa
```
...
[ Redo with entitlement norms ]

```{r}
coded_with_norms$institution = fct_recode(coded_with_norms$situation,
                                         "institution"="urgency",
                                         "non"="relationship",
                                         "non"="interaction")

model_institution = glmer(formatting ~ mean_entitlement + institution +
                            mean_power + mean_distance + mean_imposition + 
                                          (1 + mean_entitlement | subject) +
                                          (1 | stimNum), 
                                        data = coded_with_norms, 
                                        control=glmerControl(optimizer="bobyqa"),
...
model_institution_reduced = glmer(formatting ~ mean_entitlement +
                            mean_power + mean_distance + mean_imposition + 
                                          (1 + mean_entitlement | subject) +
                                          (1 | stimNum), 
                                        data = coded_with_norms, 
...
model_reason_full = glmer(factor(reason_final) ~ condition +
                                   Gender + 
                          (1 + condition | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
...
model_reason_no_entitlement = glmer(factor(reason_final) ~ Gender +
                          (1 + condition | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
                        control=glmerControl(optimizer="bobyqa"),
...
anova(model_reason_full, model_reason_no_entitlement)
...
```{r}
model_repay_full = glmer(factor(Just2.ST) ~ condition +
                                   Gender + 
                          (1 | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
...
model_repay_no_entitlement = glmer(factor(Just2.ST) ~ Gender +
                          (1 | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
                        control=glmerControl(optimizer="bobyqa"),
...
anova(model_repay_full, model_repay_no_entitlement)
```


## Justification ~ Entitlement Norms
...
model_reason_full = glmer(factor(reason_final) ~ mean_entitlement +
                                   mean_power +
                                   mean_imposition +
                                   mean_distance +
                            Gender +
...
model_reason_no_entitlement = glmer(factor(reason_final) ~  mean_power +
                                   mean_imposition +
                                   mean_distance +
                                     Gender +
                          (1 + mean_entitlement  | subject) +
...
model_reason_no_power = glmer(factor(reason_final) ~  mean_entitlement +
                                   mean_imposition +
                                   mean_distance +
                                Gender +
                          (1 + mean_entitlement | subject) +
...
model_reason_no_imposition = glmer(factor(reason_final) ~  mean_power +
                                   mean_entitlement +
                                   mean_distance +
                                     Gender +
                          (1 + mean_entitlement | subject) +
...
model_reason_no_distance = glmer(factor(reason_final) ~  mean_power +
                                   mean_imposition +
                                   mean_entitlement +
                                   Gender +
                          (1 + mean_entitlement  | subject) +
...
anova(model_reason_full, model_reason_no_entitlement)
anova(model_reason_full, model_reason_no_power)
anova(model_reason_full, model_reason_no_imposition)
anova(model_reason_full, model_reason_no_distance)
...
```{r}
p1 = anova(model_reason_full, model_reason_no_entitlement)$`Pr(>Chisq)`[2]
p2 = anova(model_reason_full, model_reason_no_power)$`Pr(>Chisq)`[2]
p3 = anova(model_reason_full, model_reason_no_imposition)$`Pr(>Chisq)`[2]
p4 = anova(model_reason_full, model_reason_no_distance)$`Pr(>Chisq)`[2]
...
```{r}
model_repay_full = glmer(factor(Just2.ST) ~ mean_entitlement +
                                   mean_power +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
...
model_repay_no_entitlement = glmer(factor(Just2.ST) ~  mean_power +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_repay_no_power = glmer(factor(Just2.ST) ~  mean_entitlement +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_repay_no_imposition = glmer(factor(Just2.ST) ~  mean_power +
                                   mean_entitlement +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_repay_no_distance = glmer(factor(Just2.ST) ~  mean_power +
                                   mean_imposition +
                                   mean_entitlement +
                                   Gender + 
                          (1 | subject) +
...
anova(model_repay_full, model_repay_no_entitlement)
anova(model_repay_full, model_repay_no_power)
anova(model_repay_full, model_repay_no_imposition)
anova(model_repay_full, model_repay_no_distance)
```

#### Adjust for multiple comparisons

```{r}
p1 = anova(model_repay_full, model_repay_no_entitlement)$`Pr(>Chisq)`[2]
p2 = anova(model_repay_full, model_repay_no_power)$`Pr(>Chisq)`[2]
p3 = anova(model_repay_full, model_repay_no_imposition)$`Pr(>Chisq)`[2]
p4 = anova(model_repay_full, model_repay_no_distance)$`Pr(>Chisq)`[2]

p.adjust(c(p1, p2, p3, p4), method="holm")
```

```{r}
summary(model_repay_full)
```
...
```{r}
mean(coded_with_norms$reason_final)
mean(coded_with_norms$Just2.ST)
```
...
```{r}
coded.reduced$agree_reason = coded.reduced$Reason.SB == coded.reduced$Reason2
mean(coded.reduced$agree_reason)

preface_bound = cbind(coded.reduced$Reason.SB, coded.reduced$Reason2)
preface_kappa = cohen.kappa(preface_bound)
preface_kappa
```
...
```{r}
coded.reduced$agree_repay = coded.reduced$Repayment.SB == coded.reduced$Repayment2
mean(coded.reduced$agree_repay, na.rm=TRUE)

preface_bound = cbind(coded.reduced$Repayment.SB, coded.reduced$Repayment2)
preface_kappa = cohen.kappa(preface_bound)
preface_kappa
```
...
model_reason_full = glmer(factor(Reason2) ~ condition +
                                   Gender + 
                          (1 + condition | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
...
model_reason_no_entitlement = glmer(factor(Reason2) ~ Gender +
                          (1 + condition | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
                        control=glmerControl(optimizer="bobyqa"),
...
```{r}
model_repay_full = glmer(factor(Repayment2) ~ condition +
                                   Gender + 
                          (1 + condition | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
...
model_repay_no_entitlement = glmer(factor(Repayment2) ~ Gender +
                          (1 + condition | subject) +
                          (1 + condition | stimNum),
                        data = coded_with_norms, 
                        control=glmerControl(optimizer="bobyqa"),
...
model_reason_full = glmer(factor(Reason2) ~ mean_entitlement +
                                   mean_power +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
...
model_reason_no_entitlement = glmer(factor(Reason2) ~  mean_power +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_reason_no_power = glmer(factor(Reason2) ~  mean_entitlement +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_reason_no_imposition = glmer(factor(Reason2) ~  mean_power +
                                   mean_entitlement +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_reason_no_distance = glmer(factor(Reason2) ~  mean_power +
                                   mean_imposition +
                                   mean_entitlement +
                                   Gender + 
                          (1 | subject) +
...
```{r}
model_repay_full = glmer(factor(Repayment2) ~ mean_entitlement +
                                   mean_power +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
...
model_repay_no_entitlement = glmer(factor(Repayment2) ~  mean_power +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_repay_no_power = glmer(factor(Repayment2) ~  mean_entitlement +
                                   mean_imposition +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_repay_no_imposition = glmer(factor(Repayment2) ~  mean_power +
                                   mean_entitlement +
                                   mean_distance +
                                   Gender + 
                          (1 | subject) +
...
model_repay_no_distance = glmer(factor(Repayment2) ~  mean_power +
                                   mean_imposition +
                                   mean_entitlement +
                                   Gender + 
                          (1 | subject) +
...
```{r}
mean(coded_with_norms$Reason2)
mean(coded_with_norms$Repayment2)
```
